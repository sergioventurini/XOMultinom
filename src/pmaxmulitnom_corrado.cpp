...
double pmaxmultinom_corrado_one(const double& x, const int& size, const Rcpp::NumericVector& prob,
  ...
  if (x < 0) {
    ...
    std::vector<std::vector<double>> res = computeQk_max(x, 2, size, prob, verbose, tol);
    for (int k = 3; k < m; k++) {
      Qk_tmp = computeQk_max(x, k, size, prob, verbose, tol);
      res = multiplyUpperTriangular(res, Qk_tmp);
    }
    res = multiplyMatrix(computeQk_max(x, 1, size, prob, verbose, tol), res);
    res = multiplyMatrix(res, computeQk_max(x, m, size, prob, verbose, tol));
    return res[0][0];
  }
}
