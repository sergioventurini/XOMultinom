#' Probability mass function (PDF) of the range for a generic multinomial random
#' vector.
#'
#' This function calculates the probability mass function (i.e. the probability to be
#' exactly equal to a given value) for the range of a generic
...
#'   CDF of the range for a generic multinomial random vector.
...
#'   range, range, range and sums of order statistics", Royal Society
...
#' xseq <- 0:n
#' 
#' cdfmax <- prangemultinom(x = xseq, size = n, prob = probs, log = FALSE,
#'                        verbose = TRUE)
