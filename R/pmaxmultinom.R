...
#'   \code{values} (containing \eqn{P(\max(N_1, \ldots, N_m) \le x)}, or
...
#'
#' @export
pmaxmultinom <- function(x, size, prob, log = FALSE, verbose = TRUE) {
  ...
  if (length(prob) > 0 && length(unique(prob)) == 1) {
    res <- pmaxmultinom_bonetti(x, size, prob, log, verbose)
  } else {
