...
#
#   xomultinom_size  -- returned by maxmin_xomultinom_size.
#
# NOTE: the existing p/d functions and maxmin_xomultinom_size must be updated
...
#'
#' @seealso \code{\link{print.xomultinom_dist}}, \code{\link{summary.xomultinom_dist}},
...
#' corresponding exact probabilities (or log-probabilities) stored in a
...
#' stored in a \code{xomultinom_dist} object, including the mean, median, mode,
#' standard deviation, effective support, and a central 95\% probability
#' interval.
#'
...
#'
#' Produces a \code{ggplot2}-based plot of the exact distribution stored in a
#' \code{xomultinom_dist} object. PMFs are displayed as lollipop (spike) charts;
#' CDFs are displayed as step functions. An optional normal approximation
#' overlay can be added for diagnostic comparison.
#'
#' @param x An object of class \code{xomultinom_dist}.
...
plot.xomultinom_dist <- function(x,
  ...
  y_label <- if (x$type == "pmf") {
    if (x$log) "log P(X = x)" else "P(X = x)"
  } else {
    if (x$log) "log P(X \u2264 x)" else "P(X \u2264 x)"
  }

  df <- data.frame(x = x$x, y = x$values)
  ...
  if (add_approx && !x$log) {
    # Compute mean and variance from the PMF
    if (x$type == "cdf") {
    ...
    if (x$type == "pmf") {
      # Use finite-difference approximation of normal density scaled to PMF
      y_approx <- dnorm(x_seq, mean = mu, sd = sig)
      df_approx <- data.frame(x = x_seq, y = y_approx)
      p <- p +
        ggplot2::geom_line(
          data = df_approx,
          ggplot2::aes(x = .data$x, y = .data$y),
          colour = approx_colour, linewidth = 0.8, linetype = "dashed"
        )
    } else {
      y_approx  <- pnorm(x_seq, mean = mu, sd = sig)
      df_approx <- data.frame(x = x_seq, y = y_approx)
      p <- p +
        ggplot2::geom_line(
          data = df_approx,
...
#' Coerce a \code{xomultinom_dist} object to a data frame
#'
#' Converts the evaluation points and probability values stored in a
...
#'
#' Internal constructor used by \code{\link{maxmin_xomultinom_size}} to wrap its
#' output in a structured S3 object.
#'
#' @param sizes Named list of sample sizes as returned by
#'   \code{\link{maxmin_xomultinom_size}}.
...
#'
#' @seealso \code{\link{print.xomultinom_size}}, \code{\link{summary.xomultinom_size}},
...
#'   \eqn{n}) is displayed on a log\eqn{_{10}} scale. Useful when
#'   \eqn{n} varies over several orders of magnitude. Defaults to
#'   \code{FALSE}.
#' @param title Character string; plot title. If \code{NULL} (default), a
#'   descriptive title is generated automatically.
#' @param ... Further arguments passed to or from other methods (currently
...
#'
#' @export
plot.xomultinom_size <- function(x, log_scale = FALSE, title = NULL, ...) {
  stat_label <- if (x$type == "max") "Maximum" else "Minimum"
  x_label    <- if (x$type == "max") "Outlier probability" else "Inlier probability"
...
#' Coerce a \code{xomultinom_size} object to a data frame
#'
#' Converts the sample size results stored in a \code{xomultinom_size} object
