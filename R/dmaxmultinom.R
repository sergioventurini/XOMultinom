...
#'
#' Computes \eqn{P(\max(N_1, \ldots, N_m) = x)} at each element of \code{x}
...
#'
#' @export
dmaxmultinom <- function(x, size, prob, log.p = FALSE, verbose = TRUE) {
  ...
  if (length(prob) > 0 && length(unique(prob)) == 1) {
    res <- dmaxmultinom_bonetti(x, size, prob, FALSE, verbose)
  } else {
    res <- dmaxmultinom_corrado(x, size, prob, FALSE, verbose)
  }
