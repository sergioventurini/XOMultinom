...
#'
#' Computes the cumulative distribution function of the minimum cell count of a
...
#'   \eqn{P(\min(N_1, \ldots, N_m) \le x)} is returned; otherwise
#'   \eqn{P(\min(N_1, \ldots, N_m) > x)}.
...
#'   \eqn{P(\min(N_1, \ldots, N_m) \le x)} (or its complement or log, according
...
#'
#' @export
pminmultinom <- function(x, size, prob, lower.tail = TRUE, log.p = FALSE,
                         verbose = TRUE) {
  ...
  if (length(prob) > 0 && length(unique(prob)) == 1) {
    res <- pminmultinom_bonetti(x, size, prob, FALSE, verbose)
  } else {
    res <- pminmultinom_corrado(x, size, prob, FALSE, verbose)
  }
...
#' of the minimum cell count \eqn{\min(N_1, \ldots, N_m)} of a multinomial
...
#' \code{\link{pminmultinom}} and \code{\link{dminmultinom}} provide the CDF or
...
minmultinomcdf <- function(size, prob, verbose = TRUE) {
  ...
  if (length(prob) > 0 && length(unique(prob)) == 1) {
    res <- pminmultinom_bonetti(xseq, size, prob, FALSE, verbose)
  } else {
    res <- pminmultinom_corrado(xseq, size, prob, FALSE, verbose)
  }
