...
#'   \code{values} (containing \eqn{P(\min(N_1, \ldots, N_m) \le x)}, or
#'   log-probabilities if \code{log = TRUE}), \code{stat = "min"},
#'   \code{type = "cdf"}, \code{size}, \code{prob}, and \code{log}.
#'
#' @examples
#' m <- 4
#' n <- 60
...
#'
#' @export
pminmultinom <- function(x, size, prob, log = FALSE, verbose = TRUE) {
  ...
  if (length(prob) > 0 && length(unique(prob)) == 1) {
    res <- pminmultinom_bonetti(x, size, prob, log, verbose)
  } else {
    res <- pminmultinom_corrado(x, size, prob, log, verbose)
  }
